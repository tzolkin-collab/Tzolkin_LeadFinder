...
class ExplosionParticles {
  ...
  spawn(x, y, z, color = 0x9d5cff, count = 16) {
...
export function AuthGraphism() {
  ...
  useEffect(() => {
    ...
    const dirLight = new THREE.DirectionalLight(0x9d5cff, 2.2);
    ...
    const wingletMat = new THREE.MeshStandardMaterial({ color: 0x9d5cff, roughness: 0.1, metalness: 0.9 });
    ...
    const gameLoop = (time) => {
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (hit) {
          const explosionColor = enemy.type === 'suicide' ? 0xff2255 : (enemy.type === 'orb' ? 0x00ffff : 0x9d5cff);
