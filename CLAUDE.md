...
> ⚠️ **Correção de 29/07 — "exemplo é marcado como exemplo" não era verdade na tela, só no dado.** Auditoria em `packages/core` achou fabricação real em 3 arquivos (`keyword-trends.service.ts`, `social-community.client.ts`, `web-traffic-analyzer.client.ts` — corrigidos no commit `a19e27f`) e no benchmark do copiloto de pitch (também corrigido — ver ADR de fontes de dados no Notion). O achado maior: `SignalRow.js` e `MostContractedServicesWidget.js` recebiam `isExample={true}` do `feed/page.js` e só reduziam opacidade (0.7 e 0.95) — nenhum rótulo textual visível. **Resolvido no commit `e242593`**: badge "exemplo" real (`.badge-example`, tom neutro) nos dois componentes; a alegação fabricada "Amostragem 1.400+ prospecções" saiu. De quebra, achado e corrigido: `--tzolkin-cyan` era usado em 3 lugares sem nunca ter sido definido no `globals.css` (cor herdada quebrada) — token criado. **Não corrigido, por decisão explícita:** o mesmo widget usa `--tzolkin-yellow` duas vezes (ícone + primeira barra) — a skill `tracer-design` trata isso como erro, fica pendente.
