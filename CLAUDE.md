...
> ⚠️ **Correção de 29/07 — "exemplo é marcado como exemplo" não era verdade na tela, só no dado.** Auditoria em `packages/core` achou fabricação real em 3 arquivos (`keyword-trends.service.ts`, `social-community.client.ts`, `web-traffic-analyzer.client.ts` — corrigidos no commit `a19e27f`) e no benchmark do copiloto de pitch (também corrigido — ver ADR de fontes de dados no Notion). O achado maior: `SignalRow.js` e (o então) `MostContractedServicesWidget.js` recebiam `isExample={true}` do `feed/page.js` e só reduziam opacidade — nenhum rótulo textual visível. **Resolvido no commit `e242593`** (badge "exemplo" real nos componentes de mercado) e depois `97aba1c` (estendido a `HeroStat`/`KeywordChip`/`PersonMention`). De quebra, achado e corrigido: `--tzolkin-cyan` era usado em 3 lugares sem nunca ter sido definido no `globals.css` — token criado.
>
> **Correção maior, mesmo dia (commit `181faee`):** `MostContractedServicesWidget` não foi só mal rotulado — a categoria inteira estava errada. Ele tentava resumir "demanda nacional por serviço B2B", e isso não tem fonte real possível (a base canônica observa presença digital pública, nunca qual serviço foi contratado). **Removido**, substituído por `NicheSignalWidget`: % de "sem site" por categoria, agregado da base do **tenant** (não claim de mercado nacional) — fonte #1 documentada na skill `tracer-design`, testado contra o tenant de seed com números batendo. Endpoint: `GET /api/businesses/niche-signal`.
>
> A visão real do dono do produto para esse espaço: um motor editorial tipo blog, postando dossiês `.md` por nicho combinando dado real + scraping + pesquisa de mercado + abordagem/roupagem — `MarketArticleCard` já é essa peça (1 exemplar manual, "Dossiê Odontologia"). Fontes #2–4 (LinkedIn, Trends, Keyword Planner) seguem como próximo passo, não construídas ainda.
